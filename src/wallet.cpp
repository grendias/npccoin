...
// NPC: total coins staked (non-spendable until maturity)
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    // Calculate coin age reward
    {
        ...
        int64_t nReward = GetProofOfStakeReward(nCoinAge, nFees, pindexPrev->nHeight);
        if (nReward <= 0)
            return false;
...
// NPC: check 'spent' consistency between wallet and txindex
// NPC: fix wallet spent state according to txindex
...
// NPC: disable transaction (only for coinstake)
